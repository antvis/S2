...
Configure the [tooltip](/en/api/general/s2-options#tooltip) field in `s2Options`, which works on **all** cells by default
...
Add [operation item](/en/api/general/s2-options#tooltipoperation) on `Tooltip` by configuring `operation` field, support [custom](#custom-tooltip-operation item).
