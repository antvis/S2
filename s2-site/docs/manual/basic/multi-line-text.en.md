...
:::warning{title="Note"}
Before reading this chapter, please ensure you have read the [Basic Concepts](/en/manual/basic/base-concept) and [Theme Configuration](/en/manual/basic/theme) chapters, and have some familiarity with the [AntV/G](https://g.antv.antgroup.com/) rendering engine.
:::
...
In S2, you can render multi-line text through the [Style](/en/api/general/s2-options#style) configuration. If the text wraps and its height is less than the cell height, the cell height will be adjusted automatically.
...
1. If you have configured a [custom cell height](/en/manual/advanced/custom/cell-size), the adaptive height will be disabled, and the custom height will take precedence.
2. By default, the cell height is calculated based on `maxLines`. When the height is [manually resized](/en/manual/advanced/interaction/resize) or a [custom cell height](/en/manual/advanced/custom/cell-size) is set, the maximum number of lines that can be displayed will be recalculated based on the current text line height to ensure proper display. This will **override** the default `maxLines` configuration.

:::

## Getting Cell Text Status

If you need to get specific states, such as `maximum text width`, `whether the text is wrapped`, or `whether the text is overflowing`, you can call the base cell methods after getting the [cell information](/en/manual/advanced/get-cell-data). For details, please [see the API](/en/api/basic-class/base-cell).
