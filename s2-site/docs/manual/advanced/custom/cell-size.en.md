...
S2 can manually drag and drop to dynamically change the width and height of cells, and also provides default three layouts: row and column`行列等宽`,`列等宽`and`行列紧凑布局`layout ( [see examples](/zh/examples/layout/basic/#compact) )
...
You can also set the height to `0` to achieve the effect of **hiding the column header** , [see an example](/zh/examples/layout/custom#hide-columns)
