...
export class BaseFrozenRowHeader extends RowHeader {
  ...
  constructor(cfg: RowHeaderConfig) {
    ...
    this.frozenHeadGroup = this.appendChild(new Group({
  ...
  protected rowCellInRectXDir(item: Node): boolean {
    const { width, scrollX = 0, seriesNumberWidth } = this.headerConfig;
  ...
  protected rowCellInRectYDir(item: Node): boolean {
    const { viewportHeight, scrollY = 0 } = this.headerConfig;
  ...
  protected offset() {
    const { scrollX = 0, scrollY, position, seriesNumberWidth } = this.headerConfig;
  ...
  public clip(): void {
    const { width, viewportHeight, scrollX = 0, scrollY = 0, seriesNumberWidth } =
      this.headerConfig;
