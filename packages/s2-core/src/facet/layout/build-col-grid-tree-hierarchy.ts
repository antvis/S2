 ...
 */
const calculateColCollapsedState = (options: {
  ...
  const { collapseFields, collapseAll, expandDepth } =
    spreadsheet.options.style?.colCell ?? {};
 ...
 */
const processColFieldValue = (options: {
...
/**
 * 生成 grid-tree 模式的列头节点
 */
const generateGridTreeColHeaderNodes = (params: HeaderNodesParams) => {
  const {
    currentField,
    fields,
    fieldValues,
  ...
  for (const originalFieldValue of fieldValues) {
    ...
    const processed = processColFieldValue({
      fieldValue,
      currentField,
      query,
      parentNode,
    ...
    if (nodeId) {
      const isCollapsed = calculateColCollapsedState({
        spreadsheet,
        nodeId,
        currentField,
        level,
      ...
      if (isLeafNode) {
        ...
        node.colIndex = hierarchy.getIndexNodes().length - 1;
      } else {
        handler?.({
          addTotalMeasureInTotal,
          addMeasureInTotalQuery,
...
const buildNormalColGridTreeHierarchy = (params: GridHeaderParams) => {
  ...
  generateGridTreeColHeaderNodes({
    ...params,
    ...
    handler: buildColGridTreeHierarchy,
  });
};

const buildTotalColGridTreeHierarchy = (params: GridHeaderParams) => {
  const {
    addTotalMeasureInTotal,
    parentNode,
    currentField,
  ...
  if (dimensionGroup?.includes(currentField)) {
    ...
    parentNode.colIndex = hierarchy.getIndexNodes().length - 1;

    return;
  } else {
    // eslint-disable-next-line @typescript-eslint/no-use-before-define
    buildTotalColGridTreeHierarchy({
      ...params,
...
/**
 * 构建 grid-tree 模式的列头层级结构
 * grid-tree 模式特点：
 * 1. 每个维度占据独立的行（与 grid 模式相同）
 * 2. 支持展开/折叠子节点（与 tree 模式相同）
 */
export function buildColGridTreeHierarchy(params: GridHeaderParams) {
  if (params.parentNode.isTotals) {
    buildTotalColGridTreeHierarchy(params);
  } else {
    buildNormalColGridTreeHierarchy(params);
  }
}
