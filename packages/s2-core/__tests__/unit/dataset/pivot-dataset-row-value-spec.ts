 ...
 */
 ...
 describe('Pivot Mode Test When Value In Row', () => {
   ...
   let dataCfg: S2DataConfig = assembleDataCfg({
    totalData: [],
    meta: [],
    fields: {
   ...
   describe('test base dataset structure', () => {
     ...
     test('should get correct row pivot meta', () => {
       ...
       expect([...rowPivotMeta.get('浙江省').children.get('杭州市').children.keys()]).toEqual(['price']);
       ...
       expect([...rowPivotMeta.get('四川省').children.get('成都市').children.keys()]).toEqual(['price']);
     });
     ...
     test('should get correct sorted dimension value', () => {
       ...
       expect([...sortedDimensionValues.keys()]).toEqual(['province', 'city', EXTRA_FIELD, 'type', 'sub_type']);
       ...
       expect([...sortedDimensionValues.get('type')]).toEqual(['家具', '办公用品']);
       expect([...sortedDimensionValues.get('sub_type')]).toEqual(['桌子', '沙发', '笔', '纸张']);
   ...
   describe('test for query data', () => {
     ...
     test('getDimensionValues function', () => {
       ...
       expect(dataSet.getDimensionValues('sub_type')).toEqual(['桌子', '沙发', '笔', '纸张']);
       ...
       expect(dataSet.getDimensionValues('sub_type', { 'type': '家具'})).toEqual(['桌子', '沙发']);
